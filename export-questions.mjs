...
const STAGE_LABELS = {
  1: 'Stage 1 — Opening',
  2: 'Stage 2 — Repeating Patterns',
  3: 'Stage 3 — Real Life',
  4: 'Stage 4 — Choice',
  5: 'Stage 5 — Money Direction',
};
...
function gold(text) {
  return new TextRun({ text, bold: true, color: '8B6914', size: 22 });
}
function gray(text) {
  return new TextRun({ text, color: '666666', size: 20 });
}
function archetypeTag(type) {
  return new TextRun({ text: ` [${ARCHETYPE_LABELS[type]}]`, color: '9333ea', italics: true, size: 18 });
}
...
children.push(
  new Paragraph({
    children: [new TextRun({ text: 'Oracle — Diagnostic Questions', bold: true, size: 40, color: '1a0e35' })],
    spacing: { after: 120 },
  }),
  new Paragraph({
    children: [gray('48 questions · 5 stages · bilingual (Hebrew / English)')],
    spacing: { after: 480 },
  }),
);
...
for (const q of questions) {
  ...
  if (q.stage !== currentStage) {
    ...
    children.push(
      new Paragraph({
        children: [new TextRun({ text: STAGE_LABELS[q.stage], bold: true, size: 28, color: '5b21b6' })],
        spacing: { before: 480, after: 160 },
        border: { bottom: { style: BorderStyle.SINGLE, size: 4, color: 'ddd6fe' } },
      }),
    );
  }
  ...
  children.push(
    new Paragraph({
      children: [
        new TextRun({ text: `Q${q.id}.  `, bold: true, size: 22, color: '1a0e35' }),
        new TextRun({ text: q.en.text, bold: true, size: 22, color: '1a0e35' }),
      ],
      spacing: { before: 240, after: 80 },
    }),
  );

  // Hebrew text (right-to-left hint via bidirectional)
  children.push(
    new Paragraph({
      children: [new TextRun({ text: q.he.text, size: 20, color: '555555', italics: true })],
      spacing: { after: 120 },
      bidirectional: false,
    }),
  );
  ...
  for (const opt of q.en.options) {
    children.push(
      new Paragraph({
        children: [
          gold(`  ${opt.label}.  `),
          new TextRun({ text: opt.text, size: 20, color: '1a0e35' }),
          archetypeTag(opt.type),
        ],
        spacing: { after: 40 },
        indent: { left: 360 },
      }),
  ...
  // Hebrew options
  children.push(new Paragraph({ children: [gray('  — Hebrew —')], spacing: { before: 60, after: 40 }, indent: { left: 360 } }));
  for (const opt of q.he.options) {
    children.push(
      new Paragraph({
        children: [
          gold(`  ${opt.label}.  `),
...
const outputPath = join(__dirname, 'Oracle_Diagnostic_Questions.docx');
const buffer = await Packer.toBuffer(doc);
writeFileSync(outputPath, buffer);
console.log(`\n✅  Saved: ${outputPath}\n    ${questions.length} questions across ${Object.keys(STAGE_LABELS).length} stages\n`);
